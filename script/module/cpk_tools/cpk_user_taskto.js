    ...
    var CPK_user_taskto = {
        createObj : function(otherObj){
            ...
            function addEvent(){
              ...
              $("input[name=employtaskTime]").live("blur", function(){
                checkTaskTime();
              });
            ...
            function checkTaskTime(){
              var time    = $("input[name=employtaskTime]").val();
              var maxtime = $("input[name=max_endtime]").val();
              time = parseInt(time);
              ...
              if(typeof time == 'number' && !isNaN(time) &&  time > 0){
                    ...
                    $("input[name=employtaskTime]").val(time); 
                    tipHide(5);
                    return true;
              }
              tipShow(5);
            ...
            function checkTaskPrice(){
              balance();
              var price = $("input[name=taskReward]").val();
              ...
              if(parseInt(_cat1)==1147){
                  ...
                  if(price <  taskConfig.art_min_money){
                      tipShow(6, price >= taskConfig.art_min_money ? "" : "文案类最低不得低于"+taskConfig.art_min_money+"元");
                      return false;
                  }
                  if(typeof price == 'number' && !isNaN(price) &&  price > 0 && price >= taskConfig.art_min_money){
                    $("input[name=taskReward]").val(price); 
                    tipHide(6);
                    return true;
                  }
              }else{
                  ...
                  if(price < taskConfig.design_min_money){
                      tipShow(6, price >= taskConfig.design_min_money ? "" : "设计类最低不得低于"+taskConfig.design_min_money+"元");
                      return false;
                  }
