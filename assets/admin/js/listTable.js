...
*/
		...
		$('span[data-tdtype="edit"]', $(self)).live('click', function() {
			...
			$('<input type="text" class="form-control" value="'+s_val+'" />').width(width).focusout(function(){
				...
				if($(this).val() != s_val) {
					//settings.url = settings.url+"/id/"+s_id+"/field/"+s_name+"/val/"+$(this).val()+"/";
		...
		// 未审核任务 审核
		$('a[data-tdtype="toggle_task"]', $(self)).live('click', function(){
			var obj = this,
			s_id = $(obj).attr('rel');
			$.getJSON(settings.url, {id:s_id, field:'status', val:0}, function(result){
				if(result.status == 1) {
					$(obj).html('已审核');
		...
		// 认证申请的审核
		$('img[data-tdtype="toggleb"]', $(self)).live('click', function(){
			var img   = this;
			var o_val = parseInt($(img).attr('data-value'));
			if(o_val == 1){
				var s_val = 2;
			}
			else if(o_val == 2){
			...
			*/
			var s_name = $(img).attr('data-field'),
			s_id = $(img).attr('data-id'),
			s_src = $(img).attr('src');
			$.getJSON(settings.url, {id:s_id, field:s_name, val:s_val}, function(result){
				if(result.status == 1) {
					var new_src = 'static/images/admin/toggle_'+s_val+'.gif';
					$(img).attr({'src':new_src,'data-value':s_val});
				}else{
					alert(result.msg)
				}
				//console.log(o_val);
		...
		// 提现申请审核
		$('img[data-tdtype="togglec"]', $(self)).live('click', function(){
			var img   = this;
			var o_val = parseInt($(img).attr('data-value'));
			if(o_val == 0){
				var s_val = 1;
		...
		// 认证申请的审核
		$('a[data-tdtype="togglec"]', $(self)).live('click', function(){
			var aObj   = this;
			var o_val = parseInt($(aObj).attr('data-value'));
			if(o_val == 1){
				var s_val = 0;
				var str = '否';
			} else{
			...
			var s_name = $(aObj).attr('data-field'),
			s_id   = $(aObj).attr('data-id'),
			s_src  = $(aObj).attr('src');
			$.getJSON(settings.url, {id:s_id, field:s_name, val:s_val}, function(result){
				if(result.status == 1) {
					$(aObj).attr({'data-value':s_val});
					$(aObj).html(str);
					$(aObj).toggleClass('a_no');
				}
			});
